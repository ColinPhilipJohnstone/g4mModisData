...
from MODIS data and saves the output as GeoTIFF files and Excel spreadsheets.

Usage:
  python extract_country_maps.py --country <CountryName> --yearstart <StartYear> --yearend <EndYear> [--onjupyter]

Arguments:
  --country   Country name (string)
  --yearstart Start year, must be >=2003 (integer) 
  --yearend   End year, must be <=2024 (integer)
  --onjupyter Optional flag if running in Jupyter

Example:
  python extract_country_maps.py --country Kenya --yearstart 2003 --yearend 2010
'''
...
#--------------------------------------------------------------------------------
# BASIC CLI CODE TO ALLOW PARAMETERS TO BE PASSED IN FROM THE COMMAND LINE
# These will be treated as global within the script (messy but simple)

def parse_args():
    parser = argparse.ArgumentParser(
        description="Simple CLI with country, yearstart, yearend, and onjupyter parameters."
    )
    ...
    parser.add_argument(
        "--onjupyter",
        action="store_true",
        help="Set this flag if running on Jupyter"
    )
...
# Main dir (depending on whether running on Jupyter or not)
# the intention is that normally this would be run from g4mModisData/ within the modis/ 
# directory but if on Jupyter then want to point to modis/ directly
if args.onjupyter:
...
def main():
    ...
    # create the bounding box for the country
    rows = np.where(np.any(country_mask, axis=1))[0]
    cols = np.where(np.any(country_mask, axis=0))[0]
    row_start, row_stop = rows[0], rows[-1] + 1
    col_start, col_stop = cols[0], cols[-1] + 1
    country_mask_crop = country_mask[row_start:row_stop, col_start:col_stop]
    del country_mask
    gc.collect()
    ...
    for year in range(yearstart, yearend + 1):
        ...
        # remove arrays that are not used anymore to save memory
        del global_map_landcover
        del country_mask_landcover
        gc.collect()
        ...
        for startday in startdays:
            ...
            # npp
            _print(f"Constructing maps for NPP.")
            print('a')
            ...
            # lai
            _print(f"Constructing maps for LAI.")
            ...
            # fpar
            _print(f"Constructing maps for FPAR.")
